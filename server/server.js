...
const allowedOrigins = [
  ...
  process.env.EXTERNAL_IP ? `http://${process.env.EXTERNAL_IP}` : null,
  process.env.EXTERNAL_IP ? `http://${process.env.EXTERNAL_IP}:80` : null
].filter(Boolean);
...
app.use(cors({
  origin: function (origin, callback) {
    ...
    // В продакшене разрешаем все для упрощения, в разработке проверяем список
...
// ==================== MIDDLEWARE ====================
// Парсинг JSON и URL-encoded (уже настроено выше)
